...
import { ShareControls } from "@/components/ui/share-controls";
...
/**
 * Single inline article image with CSS float for editorial layout.
 */
...
/**
 * Renders article body text with inline images distributed throughout the text flow.
 * Images float left or right on desktop, stacking above text on mobile.
 * The body's top-level blocks are split into segments and images are interleaved.
 */
function ArticleBodyWithImages({
  body,
  images,
  articleTitle,
}: {
  ...
  // Separate center images from floatable ones
  ...
  // Get the top-level block nodes from the Tiptap document
  const blocks = body.content ?? [];

  if (floatable.length === 0 && centerImages.length === 0) {
    // No images — just render the body
    return (
  ...
  // Split blocks into segments for interleaving floatable images.
  // We want image1 after ~1/5 of blocks, image2 after ~2/5, etc.
  const totalFloatable = floatable.length;
  const segments: JSONContent[][] = [];
  const imagesPerSegment: (ArticleImage | null)[] = [];

  // Create segments with one image each
  const blocksPerSegment = Math.max(1, Math.ceil(blocks.length / (totalFloatable + 1)));
  let blockIndex = 0;

  for (let imgIdx = 0; imgIdx < totalFloatable; imgIdx++) {
    // Take blocks for this segment
  ...
  // Remaining blocks go in the final segment
    ...
    <div className="article-editorial-body">
      {segments.map((segment, segIdx) => (
        <div key={segIdx} className="article-editorial-segment">
          {/* Render this block of text */}
          {segment.map((block, blockIdx) => (
            <RichTextBlock key={`${segIdx}-${blockIdx}`} node={block} />
          ))}

          {/* Render the image that follows this segment (if any) */}
      ...
      {/* Center images render as full-width breaks at the end */}
